...
public class Main
{
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        for(int i = 1; i <= testCase; i++)
        {
            ...
            if(!crossLimit(start, end, solve) &&  isLimitX(leftTop, rightTop, solve))
            {
            ...
            if(!crossLimit(start, end, solve) && isLimitY(leftBottom, leftTop, solve))
            {
                System.out.println("T");
                //writer(write, "T");
                continue;
            ...
            if(!crossLimit(start, end, solve) && isLimitX(leftBottom, rightBottom, solve))
            {
                System.out.println("T");
              //  writer(write, "T");
                continue;
            ...
            //print(rightBottom);
            //print(rightTop);
            if(!crossLimit(start, end, solve) && isLimitY(rightBottom, rightTop, solve))
            {
                System.out.println("T");
                //writer(write, "T");
                continue;
            ...
            if(isInSideRect(leftTop, leftBottom, rightBottom, rightTop, start, end))
            {
    ...
    private static boolean isInSideRect(Point leftTop, Point leftBottom, Point rightBottom, Point rightTop, Point start, Point end)
    {

        if(start.x == end.x && start.y == end.y &&  isLimitX(leftBottom, rightBottom, start) && isLimitY(leftTop, leftBottom, start) )return true;
        if(isLimitY(leftTop, leftBottom, start) && isLimitY(leftTop, leftBottom, end) && isLimitX(leftBottom, rightBottom, start) && isLimitX(leftBottom, rightBottom, end))return true;
        return false;

    }

    private static boolean isLimitX(Point start , Point end, Point solve)
    {
        if(start.y == end.y && (start.x <= solve.x && solve.x <= end.x) || (start.x >= solve.x && end.x <= solve.x)) return true;
        if((start.x <= solve.x && solve.x <= end.x) || (start.x >= solve.x && end.x <= solve.x))return true;
        return false;

    }

    private static boolean isLimitY(Point start, Point end, Point solve)
    {
        if(start.x == end.x && (start.y <= solve.y && solve.y <= end.y) || (start.y >= solve.y && end.y <= solve.y)) return true;
        if((start.y <= solve.y && solve.y <= end.y) || (start.y >= solve.y && end.y <= solve.y))return true;
        return false;
    ...
    {
        //System.out.println(start + " " + solve + " " + end);
