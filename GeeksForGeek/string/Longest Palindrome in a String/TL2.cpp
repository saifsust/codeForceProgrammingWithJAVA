 string longestPalin (string S) {
    ...
    pair<int, string> LPS(string str, int start, int end, vector<vector<pair<int, string>>> &memory){
        ...
        pair<int, string> mid = LPS(str, start + 1, end  - 1 , memory);
        pair<int, string> left = LPS(str, start + 1, end, memory);
        pair<int, string> right = LPS(str, start , end - 1, memory);
        
        
        if(mid.first > left.first && mid.first > right.first) return memory[start][end] = mid;
        
        if(left.first > mid.first && left.first > right.first) return memory[start][end] = left;
